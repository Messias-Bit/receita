exports.handler = async function(event, context) {
    ...
    try {
        ...
        scriptCode += `    global MainGui := Gui("+AlwaysOnTop -DPIScale")\n`;
        ...
        scriptCode += `    MainGui.SetFont("s10 normal")\n`;
        ...
        scriptCode += `    speedSlider.OnEvent("Change", (*) => {\n`;
        scriptCode += `        value := speedSlider.Value\n`;
        scriptCode += `        sleepTime := value * 1000\n`;
        scriptCode += `        speedText.Text := Format("Intervalo: {1}.0 segundos", value)\n`; // Corrigido para usar .Text e Format
        scriptCode += `    })\n\n`;
        ...
        scriptCode += `    static texts := textArray\n\n`;
        
        scriptCode += `    if (isPaused || waitingAnswer)\n`;
        scriptCode += `        return\n\n`;
        
        scriptCode += `    if (textIndex > texts.Length) {\n`;
        ...
        scriptCode += `    text := texts[textIndex]\n`;
        scriptCode += `    SendText(text)\n\n`;
        
        scriptCode += `    if (textIndex == 3) {\n`;
        scriptCode += `        waitingAnswer := textIndex\n`;
        scriptCode += `        ShowQuestion()\n`;
        scriptCode += `        return\n`;
        scriptCode += `    }\n\n`;
        
        scriptCode += `    textIndex++\n`;
        scriptCode += `    SetTimer(() => SendNextText(), sleepTime)\n`;
        scriptCode += `}\n\n`;

        scriptCode += `SendText(text) {\n`;
        scriptCode += `    if (!text)\n`;
        scriptCode += `        return\n`;
        scriptCode += `    A_Clipboard := text\n`;
        scriptCode += `    Sleep(200)\n`;
        scriptCode += `    Send("^v")\n`;
        scriptCode += `    Sleep(200)\n`;
        scriptCode += `    Send("{Enter}")\n`;
        scriptCode += `}\n\n`;
        
        scriptCode += `CreateQuestionGui() {\n`;
        scriptCode += `    global QuestionGui := Gui("+AlwaysOnTop +ToolWindow -SysMenu -DPIScale")\n`;
        ...
        scriptCode += `ShowConfigGUI() {\n`;
        scriptCode += `    global ConfigGui := Gui("+AlwaysOnTop -DPIScale")\n`;
        ...
        scriptCode += `    global WarningGui := Gui("+AlwaysOnTop -DPIScale")\n`;
        ...
        scriptCode += `    global username\n`;
        scriptCode += `    newUsername := nickEdit.Value\n\n`;
        
        scriptCode += `    if (newUsername == "") {\n`;
        scriptCode += `        UpdateStatusMessage("Erro: Digite um nickname válido")\n`;
        scriptCode += `        return\n`;
        scriptCode += `    }\n\n`;
        
        scriptCode += `    username := newUsername\n`;
        scriptCode += `    ConfigGui.Hide()\n`;
        scriptCode += `    if (!FileExist("config.ini")) {\n`;
        scriptCode += `        IniWrite(username, "config.ini", "Settings", "username")\n`;
        scriptCode += `        ShowWarningGUI()\n`;
        scriptCode += `    }\n`;
        scriptCode += `}\n\n`;
        
        scriptCode += `ConfirmWarning() {\n`;
        scriptCode += `    WarningGui.Hide()\n`;
        scriptCode += `}\n\n`;
        
        scriptCode += `UpdateStatusMessage(text) {\n`;
        scriptCode += `    statusText.Text := text\n`;
        scriptCode += `    if (text == "")\n`;
        scriptCode += `        return\n`;
        scriptCode += `    FadeIn(statusText)\n`;
        scriptCode += `}\n\n`;
        ...
        scriptCode += `        SetTimer(SendNextText, 0)\n`;
        scriptCode += `        MainGui["StartButton"].Enabled := true\n`;
        scriptCode += `        MainGui["PauseButton"].Enabled := false\n`;
        scriptCode += `        UpdateStatusMessage("Script finalizado")\n`;
        scriptCode += `        return\n`;
        scriptCode += `    }\n\n`;
