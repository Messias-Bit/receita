exports.handler = async function(event, context) {
    ...
    try {
        ...
        scriptCode += `Param1 = %A_EventInfo%\n`;
        scriptCode += `StringLeft, message, Param1, 100\n`;
        ...
        scriptCode += `if (UserNickname = "" || UserNickname = " " || UserNickname ~= "[\\/:\\*\\?\\\"<>\\|]") {\n`;
        ...
        scriptCode += `    SetEnv, A_EventInfo, "Erro: Nome de usuário não configurado"\n`; 
        scriptCode += `    SetTimer, UpdateStatusMessage, -1\n`;  
        scriptCode += `    return\n`;
        scriptCode += `}\n`;
        scriptCode += `GuiControl, Disable, StartButton\n`;
        scriptCode += `GuiControl, Enable, PauseButton\n`;
        scriptCode += `isPaused := 0\n`;
        scriptCode += `SetEnv, A_EventInfo, "Script será iniciado em 5 segundos..."\n`;  
        scriptCode += `SetTimer, UpdateStatusMessage, -1\n`;  
        scriptCode += `Sleep, 5000\n`;
        scriptCode += `SetEnv, A_EventInfo, "Script ativo"\n`; 
        scriptCode += `SetTimer, UpdateStatusMessage, -1\n`; 
        ...
        scriptCode += `    SetEnv, A_EventInfo, "Script Pausado"\n`;  
        scriptCode += `    SetTimer, UpdateStatusMessage, -1\n`;  a
        scriptCode += `} else {\n`;
        scriptCode += `    SetEnv, A_EventInfo, "Script será reiniciado em 5 segundos..."\n`;  
        scriptCode += `    SetTimer, UpdateStatusMessage, -1\n`;  
        scriptCode += `    Sleep, 5000\n`;
        scriptCode += `    isPaused := 0\n`;
        ...
        scriptCode += `SetEnv, A_EventInfo, "Script pausado, aguardando resposta do aluno..."\n`; 
        scriptCode += `SetTimer, UpdateStatusMessage, -1\n`; 
        scriptCode += `return\n\n`;
        ...
        scriptCode += `    SetEnv, A_EventInfo, "Script finalizado"\n`;  
        scriptCode += `    SetTimer, UpdateStatusMessage, -1\n`;  
        scriptCode += `    return\n`;
        scriptCode += `}\n\n`;
