exports.handler = async function(event, context) {
    ...
    try {
        ...
        if (containsUsername(inputText)) {
            scriptCode += `#Persistent\n`;
            ...
            scriptCode += `CheckConfig() {\n`;
            scriptCode += `    if (!FileExist(configFile)) {\n`;
            scriptCode += `        ShowNicknameGui()\n`;
            scriptCode += `        return false\n`;
            scriptCode += `    }\n`;
            scriptCode += `    IniRead, userNickname, %configFile%, Settings, Nickname\n`;
            scriptCode += `    if (userNickname = "ERROR" || userNickname = "") {\n`;
            scriptCode += `        ShowNicknameGui()\n`;
            scriptCode += `        return false\n`;
            scriptCode += `    }\n`;
            ...
            scriptCode += `    Gui, 2:Add, Text,, Configure seu Nickname\n`;
            scriptCode += `    Gui, 2:Font, s10 normal\n`;
            scriptCode += `    Gui, 2:Add, Text,, Digite seu nickname (máx. 25 caracteres):\n`;
            scriptCode += `    Gui, 2:Add, Edit, vNicknameInput w200 Limit25\n`;
            scriptCode += `    Gui, 2:Add, Button, gSaveNickname w100, Confirmar\n`;
            scriptCode += `    Gui, 2:Show, w300 h150\n`;
            scriptCode += `}\n\n`;

            scriptCode += `SaveNickname() {\n`;
            scriptCode += `    Gui, 2:Submit\n`;
            scriptCode += `    if (NicknameInput = "") {\n`;
            scriptCode += `        MsgBox, 16, Erro, Por favor, digite um nickname válido.\n`;
            scriptCode += `        ShowNicknameGui()\n`;
            scriptCode += `        return\n`;
            scriptCode += `    }\n`;
            scriptCode += `    if (StrLen(NicknameInput) > 25) {\n`;
            scriptCode += `        MsgBox, 16, Erro, O nickname não pode ter mais que 25 caracteres.\n`;
            scriptCode += `        ShowNicknameGui()\n`;
            scriptCode += `        return\n`;
            scriptCode += `    }\n`;
            scriptCode += `    IniWrite, %NicknameInput%, %configFile%, Settings, Nickname\n`;
            scriptCode += `    userNickname := NicknameInput\n`;
            scriptCode += `    Gui, 2:Destroy\n`;
            scriptCode += `    MsgBox, 64, Aviso Importante, O arquivo config.ini guarda suas configurações. Não o exclua.\n`;
            scriptCode += `    ShowMainGui()\n`;
            scriptCode += `}\n\n`;
        
            scriptCode += `ShowMainGui() {\n`;
            scriptCode += `    if (!CheckConfig()) {\n`;
            scriptCode += `        return\n`;
            scriptCode += `    }\n`;
        ...
        processedLines.forEach(line => {
            let processedLine = containsUsername(inputText) ? line.replace('{username}', '%" . userNickname . "%') : line;
